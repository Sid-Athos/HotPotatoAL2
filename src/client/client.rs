...
impl Client {
    ...
    fn check_stream(&mut self, stream: io::Result<TcpStream>) -> Result<(), ClientConnexionError> {
        return match stream {
            Ok(ok) => {
                ...
                println!("You are connected on {} server", self.server_connected_ip);
                Ok(())
            }
            Err(_) => {
                println!("Error when trying to connect on {}", self.server_connected_ip);
                Err(ClientConnexionError)
            }
        }
    }
    ...
    fn read_message_size(&mut self) -> Result<u32, ClientConnexionError> {
        let mut buf_n = [0u8; 4];
        let res = self.stream.take().ok_or(ClientConnexionError)?.read_exact(&mut buf_n);
        match res {
            Ok(_) => {
    ...
    fn read_message_content(&mut self, message_size: u32) -> Result<String, ClientConnexionError> {
        ...
        let res = self.stream.take().ok_or(ClientConnexionError)?.read_exact(&mut buf);
        match res {
            Ok(_) => {
